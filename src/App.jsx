  ...
  return (
    (
      <div className="row row-cols-1 row-cols-md-3 row-cols-xl-4 g-4">
        {
          movies.map(movie => <Movie movie={movie} key={movie.id} onSelectMovie={onSelectMovie} selectedMovie={selectedMovie} />)
        }
      </div>
    )
  );
        ...
        <div className="border p-2 mb-3">
          <div className="row">
            <div className="col-4">
              <img src={
                movie.poster_path ?
                  `https://media.themoviedb.org/t/p/w440_and_h660_face` + movie.poster_path : "/img/no-image.jpg"
              ...
              </p>
              {
                !isAddedToList ? (
                  <>
                    ...
                    <button className="btn btn-primary me-1" onClick={() => handleAddToList(movie)}>Listeye Ekle</button>
                  </>
                ) : (
                  <p>Film listenizde. Değerlendirmeniz: <i className="bi bi-stars text-warning me-2"></i> {selectedMovieUserRating}</p>
                )
              }
...
function MyMovieList({ selectedMovies, onDeleteFromList }) {
  ...
  return (
    selectedMovies.map(movie => <MyListMovie movie={movie} key={movie.id} onDeleteFromList={onDeleteFromList} />)
  );
}
      ...
      <div className="row">
        ...
        <div className="col-8">
          <div className="card-body">
            ...
            <button className="btn btn-danger" onClick={() => onDeleteFromList(movie.id)}>Sil</button>
          </div>
        </div>
      </div>
    </div>
